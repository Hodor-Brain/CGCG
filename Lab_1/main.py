...
if __name__ == "__main__":
    points_list = [Point(2, 5), Point(7, 2), Point(3, 9), Point(6, 6), Point(10, 2),
                  Point(9, 8), Point(4, 7), Point(3, 7), Point(1, 9)]
    search_list = [Point(3, 4), Point(7, 8)]
    tree = Tree(points_list)
    #tree.graph_viz()
    res = tree.find(search_list)
    print(len(res))

    figure, axes = plt.subplots(nrows=1, ncols=1, figsize=(10, 10))
    tree.plot_points(figure, axes, res)
